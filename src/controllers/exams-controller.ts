...
const getExamsByLevel = async (req: Request, res: Response): Promise<void> => {
    await handleAsyncError(async () => {
        ...
        const examsByDateAndTime: { [date: string]: { [time: string]: ExamDocument[] } } = exams.reduce((acc, exam) => {
            const examDate = exam.date;
            ...
            // Arrange exam times in descending order for each date
            acc[examDate][examTime].unshift(exam); // Using unshift to add to the beginning
